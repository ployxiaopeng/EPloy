...
namespace EPloy
{
    public class ProcedureSwitchScene : ProcedureBase
    {
        ...
        {
            base.OnEnter();
            HotFixMudule.Event.Subscribe(FrameEvent.LoadSceneEvt, OnLoadSceneEvt);
            ...
            // 卸载所有场景
            string[] loadedSceneAssetNames = HotFixMudule.Scene.GetLoadedSceneAssetNames();
            for (int i = 0; i < loadedSceneAssetNames.Length; i++)
                HotFixMudule.Scene.UnloadScene(loadedSceneAssetNames[i]);
            ...
            HotFixMudule.Scene.LoadScene(AssetUtility.GetSceneAsset(NextScene));
        }
        ...
        {
            HotFixMudule.Event.Unsubscribe(FrameEvent.LoadSceneEvt, OnLoadSceneEvt);
