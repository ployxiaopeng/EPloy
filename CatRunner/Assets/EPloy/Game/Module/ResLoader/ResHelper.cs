...
namespace EPloy.Res
{
    ...
    public class ResHelper
    {
        ...
        private IEnumerator LoadBytesCo(string fileUri, LoadBytesCallbacks loadBytesCallbacks)
        {
            ...
            {
                loadBytesCallbacks.LoadBytesFailureCallback(fileUri, unityWebRequest.error);
            }
            else
            {
        ...
        private IEnumerator LoadBinaryCo(string fileUri, ResInfo resInfo, LoadBinaryCallbacks loadBinaryCallbacks)
        {
            ...
            if (unityWebRequest.result == UnityWebRequest.Result.ConnectionError)
            {
                loadBinaryCallbacks.LoadBinaryFailureCallback?.Invoke(fileUri, LoadResStatus.AssetError, unityWebRequest.error);
            }
            else
            {
                bytes = unityWebRequest.downloadHandler.data;
