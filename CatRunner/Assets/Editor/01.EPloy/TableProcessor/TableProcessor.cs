...
namespace EPloy.Editor.DataTableTools
{
    public sealed partial class DataTableProcessor
    {
        ...
        public DataTableProcessor(string dataTableFileName, Encoding encoding, int nameRow, int typeRow, int? defaultValueRow, int? commentRow, int contentStartRow, int idColumn)
        {
            ...
            {
                 Log.Fatal(UtilText.Format("Data table file '{0}' is not a txt.", dataTableFileName));
            }

            if (!File.Exists(dataTableFileName))
            {
                 Log.Fatal(UtilText.Format("Data table file '{0}' is not exist.", dataTableFileName));
            }
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                {
                     Log.Fatal(UtilText.Format("Table:{3},Raw Column is '{1}', but line '{0}' column is '{2}'.", i.ToString(), rawColumnCount.ToString(), rawValue.Length.ToString(), dataTableFileName));
                }
            ...
            if (nameRow < 0)
            {
                 Log.Fatal(UtilText.Format("Name row '{0}' is invalid.", nameRow.ToString()));
            }

            if (typeRow < 0)
            {
                 Log.Fatal(UtilText.Format("Type row '{0}' is invalid.", typeRow.ToString()));
            }

            if (contentStartRow < 0)
            {
                 Log.Fatal(UtilText.Format("Content start row '{0}' is invalid.", contentStartRow.ToString()));
            }

            if (idColumn < 0)
            {
                 Log.Fatal(UtilText.Format("Id column '{0}' is invalid.", idColumn.ToString()));
            }

            if (nameRow >= rawRowCount)
            {
                 Log.Fatal(UtilText.Format("Name row '{0}' >= raw row count '{1}' is not allow.", nameRow.ToString(), rawRowCount.ToString()));
            }

            if (typeRow >= rawRowCount)
            {
                 Log.Fatal(UtilText.Format("Type row '{0}' >= raw row count '{1}' is not allow.", typeRow.ToString(), rawRowCount.ToString()));
            }

            if (defaultValueRow.HasValue && defaultValueRow.Value >= rawRowCount)
            {
                 Log.Fatal(UtilText.Format("Default value row '{0}' >= raw row count '{1}' is not allow.", defaultValueRow.Value.ToString(), rawRowCount.ToString()));
            }

            if (commentRow.HasValue && commentRow.Value >= rawRowCount)
            {
                 Log.Fatal(UtilText.Format("Comment row '{0}' >= raw row count '{1}' is not allow.", commentRow.Value.ToString(), rawRowCount.ToString()));
            }

            if (contentStartRow > rawRowCount)
            {
                 Log.Fatal(UtilText.Format("Content start row '{0}' > raw row count '{1}' is not allow.", contentStartRow.ToString(), rawRowCount.ToString()));
            }

            if (idColumn >= rawColumnCount)
            {
                 Log.Fatal(UtilText.Format("Id column '{0}' >= raw column count '{1}' is not allow.", idColumn.ToString(), rawColumnCount.ToString()));
            }
        ...
        public bool IsIdColumn(int rawColumn)
        {
            ...
            {
                 Log.Fatal(UtilText.Format("Raw column '{0}' is out of range.", rawColumn.ToString()));
            }
        ...
        public bool IsCommentRow(int rawRow)
        {
            ...
            {
                 Log.Fatal(UtilText.Format("Raw row '{0}' is out of range.", rawRow.ToString()));
            }
        ...
        public bool GenerateDataFile(string outputFileName, Encoding encoding)
        {
            ...
            try
            {
                        ...
                        for (int i = ContentStartRow; i < RawRowCount; i++)
                        {
                            ...
                            for (int j = 0; j < RawColumnCount; j++)
                            {
                                ...
                                catch
                                {
                                    ...
                                    {
                                        Debug.LogError(UtilText.Format("Parse raw value failure. OutputFileName='{0}' RawRow='{1}' RowColumn='{2}' Name='{3}' Type='{4}' RawValue='{5}'", outputFileName, i.ToString(), j.ToString(), GetName(j), GetLanguageKeyword(j), GetValue(i, j)));
                                        return false;
                                    }
                                    else
                                    {
                                        Debug.LogWarning(UtilText.Format("Parse raw value failure, will try default value. OutputFileName='{0}' RawRow='{1}' RowColumn='{2}' Name='{3}' Type='{4}' RawValue='{5}'", outputFileName, i.ToString(), j.ToString(), GetName(j), GetLanguageKeyword(j), GetValue(i, j)));
                                        try
                                        {
                                        ...
                                        {
                                            Debug.LogError(UtilText.Format("Parse default value failure. OutputFileName='{0}' RawRow='{1}' RowColumn='{2}' Name='{3}' Type='{4}' RawValue='{5}'", outputFileName, i.ToString(), j.ToString(), GetName(j), GetLanguageKeyword(j), GetComment(j)));
                                            return false;
                                        }
                                    }
                                }
                ...
                Debug.Log(UtilText.Format("Parse data table '{0}' success.", outputFileName));
                return true;
            }
            catch (Exception exception)
            {
                Debug.LogError(UtilText.Format("Parse data table '{0}' failure, exception is '{1}'.", outputFileName, exception.ToString()));
                return false;
            }
        }
        ...
        public bool SetCodeTemplate(string codeTemplateFileName, Encoding encoding)
        {
            try
            {
                ...
                Debug.Log(UtilText.Format("Set code template '{0}' success.", codeTemplateFileName));
                return true;
            }
            catch (Exception exception)
            {
                Debug.LogError(UtilText.Format("Set code template '{0}' failure, exception is '{1}'.", codeTemplateFileName, exception.ToString()));
                return false;
            }
        }
        ...
        public bool GenerateCodeFile(string outputFileName, Encoding encoding, object userData = null)
        {
            ...
            try
            {
                ...
                Debug.Log(UtilText.Format("Generate code file '{0}' success.", outputFileName));
                return true;
            }
            catch (Exception exception)
            {
                Debug.LogError(UtilText.Format("Generate code file '{0}' failure, exception is '{1}'.", outputFileName, exception.ToString()));
                return false;
            }
        }
    }
