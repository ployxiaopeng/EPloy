...
namespace EPloy.Game
{
    ...
    public class FileSystemModule : IGameModule
    {
        ...
        public bool HasFileSystem(string fullPath)
        {
            ...
            return FileSystems.ContainsKey(UtilPath.GetRegularPath(fullPath));
        }
        ...
        public IFileSystem GetFileSystem(string fullPath)
        {
            ...
            if (FileSystems.TryGetValue(UtilPath.GetRegularPath(fullPath), out fileSystem))
            {
                return fileSystem;
            }
            ...
            int maxBlockCount)
        {
            ...
            fullPath = UtilPath.GetRegularPath(fullPath);
            if (FileSystems.ContainsKey(fullPath))
            {
                Log.Fatal(UtilText.Format("File system '{0}' is already exist.", fullPath));
                return null;
            }
            ...
            {
                Log.Fatal(UtilText.Format("Create file system stream for '{0}' failure.", fullPath));
                return null;
            }
            ...
            if (fileSystem == null)
            {
                Log.Fatal(UtilText.Format("Create file system '{0}' failure.", fullPath));
                return null;
            }
        ...
        public IFileSystem LoadFileSystem(string fullPath, FileSystemAccess access)
        {
            ...
            if (fileSystem == null)
            {
                Log.Fatal(UtilText.Format("Load file system '{0}' failure.", fullPath));
                return null;
            }
