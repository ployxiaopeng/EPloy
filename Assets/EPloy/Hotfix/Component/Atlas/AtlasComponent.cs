...
namespace EPloy
{
    ...
    {
        private IDataTable<DRResSprite> DTSprite { get { return GameEntry.DataTable.GetDataTable<DRResSprite>(); } }
        ...
        public async void LoadAtlas(string atlasName)
        {
            ...
            isLoad = true;
            GameObject atlasGo = await GameEntry.Res.AwaitLoadAsset<GameObject>(Config.AtlasNames[atlasName]);
            if (atlasGo == null)
            {
            ...
            GameEntry.Event.Fire(atlasSuccessEvt);
            isLoad = false;
