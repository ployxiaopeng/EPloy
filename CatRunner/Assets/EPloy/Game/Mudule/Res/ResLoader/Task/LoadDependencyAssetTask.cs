...
namespace EPloy.Game.Res
{
    ...
    internal sealed class LoadDependAssetTask : LoadResTaskBase
    {
        ...
        public static LoadDependAssetTask Create(string assetName, ResInfo resInfo, string[] dependencyAssetNames, LoadResTaskBase mainTask)
        {
            LoadDependAssetTask loadDependencyAssetTask = ReferencePool.Acquire<LoadDependAssetTask>();
            loadDependencyAssetTask.Initialize(assetName,null, resInfo, dependencyAssetNames, null);
        ...
        public override void OnLoadAssetFailure(LoadResAgent agent, LoadResStatus status, string errorMessage)
        {
            ...
            m_MainTask.OnLoadAssetFailure(agent, LoadResStatus.DependencyError,
                UtilText.Format("Can not load dependency asset '{0}', internal status '{1}', internal error message '{2}'.",
