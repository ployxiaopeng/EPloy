...
namespace EPloy.Editor.ResourceTools
{
    ...
    internal sealed class ResourceBuilder : EditorWindow
    {
            ...
            EditorGUILayout.BeginVertical(GUILayout.Width(position.width), GUILayout.Height(position.height));
            {
                ...
                EditorGUILayout.BeginVertical("box");
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        EditorGUILayout.LabelField(m_Controller.Version);
                    }
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.BeginHorizontal();
                    {
                ...
                EditorGUILayout.BeginVertical("box");
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        GUILayout.Label(UtilText.Format("{0} ({1})", m_Controller.ApplicableGameVersion, m_Controller.InternalResourceVersion.ToString()));
                    }
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.BeginHorizontal();
                    {
        ...
        private void GetBuildMessage(out string message, out MessageType messageType)
        {
            ...
            {
                message += UtilText.Format("{0} will be overwritten.", m_Controller.OutputPackagePath);
            ...
            if (Directory.Exists(m_Controller.OutputFullPath))
            {
                ...
                message += UtilText.Format("{0} will be overwritten.", m_Controller.OutputFullPath);
                messageType = MessageType.Warning;
            }
        ...
        {
            if (EditorUtility.DisplayCancelableProgressBar("Processing AssetBundle", UtilText.Format("Processing '{0}'...", assetBundleName), progress))
            {
                EditorUtility.ClearProgressBar();
                return true;
            }
        ...
        {
            if (EditorUtility.DisplayCancelableProgressBar("Processing Binary", UtilText.Format("Processing '{0}'...", binaryName), progress))
            {
                EditorUtility.ClearProgressBar();
                return true;
            }
        ...
        {
            EditorUtility.ClearProgressBar();
            Debug.Log(UtilText.Format("Build resources for '{0}' complete.", platform.ToString()));
        }

        private void OnBuildResourceError(string errorMessage)
        {
            EditorUtility.ClearProgressBar();
            Debug.LogWarning(UtilText.Format("Build resources error with error message '{0}'.", errorMessage));
        }
    }
}
