...
namespace EPloy.Editor.ResourceTools
{
    ...
    {
        private const string DefaultSourceAssetRootPath = "Assets/Res";
        ...
        {
            m_ConfigurationPath = UtilPath.GetRegularPath(Path.Combine(Application.dataPath,EPloyEditorPath.ResEditor));
        ...
        public bool Load()
        {
            ...
            try
            {
                ...
                XmlNode xmlEditor = xmlRoot.SelectSingleNode("ResEditor");
        ...
        public bool Save()
        {
            try
            {
                ...
                XmlElement xmlEditor = xmlDocument.CreateElement("ResEditor");
        ...
        {
            return m_ResourceCollection.GetResource(fullName);
        }

        public bool HasResource(string fullName)
        {
            return m_ResourceCollection.HasResource(fullName);
        }

        public bool AddResource(string fullName, LoadType loadType)
        {
            return m_ResourceCollection.AddResource(fullName, loadType);
        }

        public bool RenameResource(string oldFullName,string newFullName)
        {
            return m_ResourceCollection.RenameResource(oldFullName, newFullName);
        }

        public bool RemoveResource(string name)
        {
            Asset[] assetsToRemove = m_ResourceCollection.GetAssets(name);
            if (m_ResourceCollection.RemoveResource(name))
            {
        ...
        {
            return m_ResourceCollection.SetResourceLoadType(name, loadType);
        }

        public int RemoveUnusedResources()
        {
            List<Resource> resources = new List<Resource>(m_ResourceCollection.GetResources());
            List<Resource> removeResources = resources.FindAll(resource => GetAssets(resource.Name).Length <= 0);
            foreach (Resource removeResource in removeResources)
            {
                m_ResourceCollection.RemoveResource(removeResource.Name);
            }
        ...
        {
            List<Asset> assets = new List<Asset>(m_ResourceCollection.GetAssets(name));
            switch (AssetSorter)
            {
        ...
        {
            if (m_ResourceCollection.AssignAsset(guid, name))
            {
        ...
        private void RefreshSourceAssetSearchPaths()
        {
            ...
            if (m_SourceAssetSearchRelativePaths.Count > 0)
            {
                foreach (string sourceAssetSearchRelativePath in m_SourceAssetSearchRelativePaths)
                {
                    m_SourceAssetSearchPaths.Add(UtilPath.GetRegularPath(Path.Combine(m_SourceAssetRootPath, sourceAssetSearchRelativePath)));
                }
            }
            else
            {
