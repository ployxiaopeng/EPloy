    ...
    internal sealed class ResourceAnalyzer : EditorWindow
    {
        ...
        private void DrawAssetDependencyViewer()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUILayout.BeginVertical(GUILayout.Width(position.width * 0.4f));
                {
                    ...
                    {
                        title = UtilText.Format("Assets In Resources ({0})", m_AssetCount.ToString());
                    }
                    else
                    {
                        title = UtilText.Format("Assets In Resources ({0}/{1})", m_CachedAssetNames.Length.ToString(), m_AssetCount.ToString());
                    }
                    ...
                    EditorGUILayout.BeginVertical("box");
                    {
                        ...
                        EditorGUILayout.LabelField("Resource Name", m_SelectedAssetName == null ? "<None>" : m_Controller.GetAsset(m_SelectedAssetName).Resource.Name);
                        EditorGUILayout.BeginHorizontal();
                        {
                ...
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(UtilText.Format("Dependency Resources ({0})", m_SelectedDependencyData.DependencyResourceCount.ToString()), EditorStyles.boldLabel);
                    EditorGUILayout.BeginVertical("box", GUILayout.Height(position.height * 0.2f));
                    {
                        m_DependencyResourcesScroll = EditorGUILayout.BeginScrollView(m_DependencyResourcesScroll);
                        {
                            ...
                            {
                                GUILayout.Label(dependencyResource.Name);
                            }
                        }
                        EditorGUILayout.EndScrollView();
                    }
                    EditorGUILayout.EndVertical();
                    EditorGUILayout.LabelField(UtilText.Format("Dependency Assets ({0})", m_SelectedDependencyData.DependencyAssetCount.ToString()), EditorStyles.boldLabel);
                    ...
                    EditorGUILayout.EndVertical();
                    EditorGUILayout.LabelField(UtilText.Format("Scattered Dependency Assets ({0})", m_SelectedDependencyData.ScatteredDependencyAssetCount.ToString()), EditorStyles.boldLabel);
                        ...
                        m_ScatteredDependencyAssetsScroll = EditorGUILayout.BeginScrollView(m_ScatteredDependencyAssetsScroll);
                        {
                            ...
                            foreach (string scatteredDependencyAssetName in scatteredDependencyAssetNames)
                            {
                                EditorGUILayout.BeginHorizontal();
                                {
                                    ...
                                    EditorGUI.EndDisabledGroup();
                                    GUILayout.Label(count > 1 ? UtilText.Format("{0} ({1})", scatteredDependencyAssetName, count.ToString()) : scatteredDependencyAssetName);
                                }
                                EditorGUILayout.EndHorizontal();
                            }
                        }
        ...
        private void DrawScatteredAssetViewer()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUILayout.BeginVertical(GUILayout.Width(position.width * 0.4f));
                {
                    ...
                    {
                        title = UtilText.Format("Scattered Assets ({0})", m_ScatteredAssetCount.ToString());
                    }
                    else
                    {
                        title = UtilText.Format("Scattered Assets ({0}/{1})", m_CachedScatteredAssetNames.Length.ToString(), m_ScatteredAssetCount.ToString());
                    }
                    EditorGUILayout.LabelField(title, EditorStyles.boldLabel);
                ...
                {
                    GUILayout.Space(5f);
                    //  EditorGUILayout.LabelField(UtilText.Format("Host Assets ({0})", m_SelectedHostAssets.Length.ToString()), EditorStyles.boldLabel);
                        ...
                        m_HostAssetsScroll = EditorGUILayout.BeginScrollView(m_HostAssetsScroll);
                        {
                            foreach (Asset hostAsset in m_SelectedHostAssets)
                            {
                                EditorGUILayout.BeginHorizontal();
                                {
                                    ...
                                    GUILayout.Label(UtilText.Format("{0} [{1}]", hostAsset.Name, hostAsset.Resource.Name));
                                }
                                EditorGUILayout.EndHorizontal();
                            }
                        }
        ...
        private void DrawCircularDependencyViewer()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(UtilText.Format("Circular Dependency ({0})", m_CircularDependencyCount.ToString()), EditorStyles.boldLabel);
                    m_CircularDependencyScroll = EditorGUILayout.BeginScrollView(m_CircularDependencyScroll);
                    {
                        ...
                        {
                            GUILayout.Label(UtilText.Format("{0}) {1}", (++count).ToString(), circularDependencyData[circularDependencyData.Length - 1]), EditorStyles.boldLabel);
                            EditorGUILayout.BeginVertical("box");
                            {
        ...
        {
            EditorUtility.DisplayProgressBar("Analyzing Assets", UtilText.Format("Analyzing assets, {0}/{1} analyzed.", index.ToString(), count.ToString()), (float)index / count);
        }
