...
namespace EPloy.ObjectPool
{
    ...
    internal class ObjectPool : ObjectPoolBase
    {
        ...
        public override void Unspawn(object target)
        {
            ...
            {
                Log.Fatal(Utility.Text.Format("Can not find target in object pool '{0}', target type is '{1}', target value is '{2}'.",
        ...
        public override void ReleaseAllUnused()
        {
            AutoReleaseTime = 0f;
            GetCanReleaseObjects(CanReleaseObjects);
            foreach (ObjectBase toReleaseObject in CanReleaseObjects)
            {
                ReleaseObject(toReleaseObject);
            }
        }
