    ...
    internal sealed partial class LoadResAgent : ITaskAgent<LoadResTaskBase>
    {
        ...
        public void LoadAsset(object resource, string assetName, Type assetType, bool isScene)
        {
            ...
            if (isScene)
            {
                ...
                {
                    string errMsg = UtilText.Format("Scene name '{0}' is invalid.", assetName);
                    OnError(LoadResStatus.AssetError, errMsg);
                    return;
                }
            ...
            {
                //Log.Error("assetName:" + assetName);
                //string[] str = assetBundle.GetAllAssetNames();
                //foreach (var s in str)
                //{
                //    Log.Error("assetBundle asset " + s);
                //}
                if (assetType != null)
                {
            ...
            if (m_UnityWebRequest != null)
            {
                if (m_UnityWebRequest.isDone)
                {
                    ...
                    else
                    {
                        ...
                        string errMsg = UtilText.Format("Can not load asset bundle '{0}' with error message '{1}'.", m_BytesFullPath, isError ? m_UnityWebRequest.error : null);
        ...
        private void UpdateFileAssetBundleCreateRequest()
        {
            if (m_FileAssetBundleCreateRequest != null)
            {
                if (m_FileAssetBundleCreateRequest.isDone)
                {
                    ...
                    {
                        m_FileAssetBundleCreateRequest = null;
                        string errMsg = UtilText.Format("Can not load asset bundle from file '{0}' which is not a valid asset bundle.", m_FileName == null ? m_FileFullPath : UtilText.Format("{0} | {1}", m_FileFullPath, m_FileName));
                        OnError(LoadResStatus.NotExist, errMsg);
                    }
                }
            }
        ...
        private void UpdateAssetBundleRequest()
        {
            if (m_AssetBundleRequest != null)
            {
                if (m_AssetBundleRequest.isDone)
                {
                    ...
                    {
                        m_AssetBundleRequest = null;
                        string errMsg = UtilText.Format("Can not load asset '{0}' from asset bundle which is not exist.", m_AssetName);
                        OnError(LoadResStatus.NotExist, errMsg);
                    }
                }
            }
        ...
        private void UpdateAsyncOperation()
        {
            if (m_AsyncOperation != null)
            {
                if (m_AsyncOperation.isDone)
                {
                    ...
                    {
                        string errMsg = UtilText.Format("Can not load scene asset '{0}' from asset bundle.", m_AssetName);
                        OnError(LoadResStatus.NotExist, errMsg);
                    }
                }
            }
