    ...
    public sealed partial class ResourceAnalyzerController
    {
        ...
        public void Analyze()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    Debug.LogWarning(UtilText.Format("Can not find asset by guid '{0}'.", assets[i].Guid));
                    continue;
                }
        ...
        private void AnalyzeAsset(string assetName, Asset hostAsset, DependencyData dependencyData, HashSet<string> scriptAssetNames)
        {
            ...
            foreach (string dependencyAssetName in dependencyAssetNames)
            {
                ...
                {
                    Debug.LogWarning(UtilText.Format("Can not find guid by asset '{0}'.", dependencyAssetName));
                    continue;
                }
