...
namespace ETModel
{
    ...
    public class ConfigHelper : IConfigHelper
    {
        ...
        public ConfigHelper(IConfigManager configManager)
        {
        ...
        {
            Init.Resource.UnloadAsset(configAsset);
        }
        ...
        /// <param name="userData">用户自定义数据。</param>
        /// <returns>是否加载成功。</returns>
        protected bool LoadConfig(string configName, object configAsset, LoadType loadType, object userData)
        {
            ...
            {
                case LoadType.Text:
                    retVal = ConfigManager.ParseConfig(textAsset.text, userData);
                    break;
                case LoadType.Bytes:
                    retVal = ConfigManager.ParseConfig(textAsset.bytes, userData);
                    break;
                case LoadType.Stream:
                    using (MemoryStream stream = new MemoryStream(textAsset.bytes, false))
                    {
                        retVal = ConfigManager.ParseConfig(stream, userData);
                    }
                    break;
                default:
        ...
        {
            return ConfigManager.AddConfig(configName, boolValue, intValue, floatValue, stringValue);
        }
