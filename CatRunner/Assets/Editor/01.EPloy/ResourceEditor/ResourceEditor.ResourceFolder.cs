    ...
    internal sealed partial class ResourceEditor : EditorWindow
    {
        private sealed class ResourceFolder
        {
            ...
            public string FromRootPath
            {
                get
                {
                    return Folder == null ? string.Empty : (Folder.Folder == null ? Name : UtilText.Format("{0}/{1}", Folder.FromRootPath, Name));
                }
            }
            ...
            public ResourceFolder GetFolder(string name)
            {
                ...
                {
                     Log.Fatal("Resource folder name is invalid.");
                }
            ...
            public ResourceFolder AddFolder(string name)
            {
                ...
                if (folder != null)
                {
                     Log.Fatal("Resource folder is already exist.");
                }
            ...
            public ResourceItem GetItem(string name)
            {
                if (string.IsNullOrEmpty(name))
                {
                     Log.Fatal("Resource item name is invalid.");
                }
            ...
            public void AddItem(string name, Resource resource)
            {
                ...
                if (item != null)
                {
                     Log.Fatal("Resource item is already exist.");
                }
