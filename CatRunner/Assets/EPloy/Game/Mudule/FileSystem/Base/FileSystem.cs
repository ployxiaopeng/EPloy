...
namespace EPloy.Game.SystemFile
{
    /// <summary>
    /// 文件系统。
    /// </summary>
    public sealed partial class FileSystem
    {
        ...
        private readonly UnOrderMultiMap<int, int> m_FreeBlockIndexes;
        ...
        private FileSystem(string fullPath, FileSystemAccess access, FileSystemStream stream)
        {
            ...
            m_FreeBlockIndexes = new UnOrderMultiMap<int, int>();
            ...
            UtilMarshal.EnsureCachedHGlobalSize(CachedBytesLength);
        }
        ...
        public static FileSystem Create(string fullPath, FileSystemAccess access, FileSystemStream stream, int maxFileCount, int maxBlockCount)
        {
            ...
            UtilMarshal.StructureToBytes(fileSystem.m_HeaderData, HeaderDataSize, CachedBytes);
        ...
        public static FileSystem Load(string fullPath, FileSystemAccess access, FileSystemStream stream)
        {
            ...
            fileSystem.m_HeaderData = UtilMarshal.BytesToStructure<HeaderData>(HeaderDataSize, CachedBytes);
            CalcOffsets(fileSystem);
            ...
            for (int i = 0; i < fileSystem.m_HeaderData.BlockCount; i++)
            {
                ...
                BlockData blockData = UtilMarshal.BytesToStructure<BlockData>(BlockDataSize, CachedBytes);
        ...
        public bool WriteFile(string name, byte[] buffer, int startIndex, int length)
        {
            ...
            {
                Log.Fatal(UtilText.Format("Name '{0}' is too long.", name));
                return false;
            }
        ...
        public bool RenameFile(string oldName, string newName)
        {
            ...
            {
                Log.Fatal(UtilText.Format("New name '{0}' is too long.", newName));
                return false;
            }
        ...
        private bool TryCombineFreeBlocks(int freeBlockIndex)
        {
            ...
            foreach (KeyValuePair<int, TypeLinkedList<int>> blockIndexes in m_FreeBlockIndexes)
            {
        ...
        {
            TypeLinkedList<int> lengthRange = default(TypeLinkedList<int>);
        ...
        private int AllocBlock(int length)
        {
            ...
            int lengthFound = -1;
            TypeLinkedList<int> lengthRange = default(TypeLinkedList<int>);
            foreach (KeyValuePair<int, TypeLinkedList<int>> i in m_FreeBlockIndexes)
            {
                if (i.Key < length)
                {
                    continue;
        ...
        private int AllocString(string value)
        {
            ...
            else
            {
                ...
                if (index < m_HeaderData.MaxFileCount)
                {
                    ...
                    UtilRandom.GetRandomBytes(bytes);
        ...
        private void WriteHeaderData()
        {
            ...
            UtilMarshal.StructureToBytes(m_HeaderData, HeaderDataSize, CachedBytes);
            m_Stream.Position = 0L;
            m_Stream.Write(CachedBytes, 0, HeaderDataSize);
        }

        private void WriteBlockData(int blockIndex)
        {
            UtilMarshal.StructureToBytes(m_BlockDatas[blockIndex], BlockDataSize, CachedBytes);
            m_Stream.Position = m_BlockDataOffset + BlockDataSize * blockIndex;
            m_Stream.Write(CachedBytes, 0, BlockDataSize);
        }

        private StringData ReadStringData(int stringIndex)
        {
            m_Stream.Position = m_StringDataOffset + StringDataSize * stringIndex;
            m_Stream.Read(CachedBytes, 0, StringDataSize);
            return UtilMarshal.BytesToStructure<StringData>(StringDataSize, CachedBytes);
        }

        private void WriteStringData(int stringIndex, StringData stringData)
        {
            UtilMarshal.StructureToBytes(stringData, StringDataSize, CachedBytes);
            m_Stream.Position = m_StringDataOffset + StringDataSize * stringIndex;
            m_Stream.Write(CachedBytes, 0, StringDataSize);
        }
