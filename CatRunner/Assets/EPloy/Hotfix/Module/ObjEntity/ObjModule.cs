...
/// 实例物体组件。
/// </summary>
public sealed partial class ObjModule : IGameModule
{
    ...
    public void ShowObj(ObjBase objData, Action<ObjBase> callBack = null, ObjGroupName objGroupName = ObjGroupName.Default)
    {
        ...
        string path = UtilAsset.GetObjAsset(objData.AssetName);
        ObjectInstance objInstance = objGroup.SpawnObjInstance(path);
        if (objInstance == null)
        {
            ObjsStartLoad.Add(serialId, objData.AssetName);
    ...
    private void LoadAssetSuccessCallback(string objAssetName, object objAsset, float duration, object userData)
    {
        ...
        ObjectInstance objInstance = EPloy.ObjectPool.ObjectInstance.Create(objAssetName, objAsset, ObjectInstance);
