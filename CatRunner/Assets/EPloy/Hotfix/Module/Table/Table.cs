...
namespace EPloy.Table
{
    ...
    public sealed class Table<T> : DataTableBase where T : IDataRow,new()
    {
        ...
        public override bool AddDataRow(byte[] dataRowBytes, int startIndex, int length)
        {
            try
            {
                T dataRow = ReferencePool.Acquire<T>();
            ...
            {
                Log.Fatal(UtilText.Format("Can not parse data row bytes for data table '{0}' with exception '{1}'.", new TypeNamePair(typeof(T), Name).ToString(), exception.ToString()));
                return false;
            }
        }
        ...
        private void InternalAddDataRow(T dataRow)
        {
            ...
            {
                Log.Fatal(UtilText.Format("Already exist '{0}' in data table '{1}'.", dataRow.Id, new TypeNamePair(typeof(T), Name).ToString()));
                return;
            }
