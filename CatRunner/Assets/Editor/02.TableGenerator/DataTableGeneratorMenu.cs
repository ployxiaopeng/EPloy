...
namespace EPloy.Editor.DataTableTools
{
    ...
    {
        [MenuItem("EPloy/Table/Generator")]
        public static void GenerateDataTables()
        {
            //文本格式标准化
            //Format();
            foreach (string dataTableName in ResConfig.DataTableNames)
            {
                DataTableProcessor dataTableProcessor = DataTableGenerator.CreateDataTableProcessor(dataTableName);
                if (!DataTableGenerator.CheckRawData(dataTableProcessor, dataTableName))
                {
                    Debug.LogError(UtilText.Format("Check raw data failure. DataTableName='{0}'", dataTableName));
                    break;
                }
        ...
        [MenuItem("EPloy/Table/TableEndClass")]
        public static void Func01()
        {
            ScriptingDefineSymbols.AddScriptingDefineSymbol(Table);
        }
        [MenuItem("EPloy/Table/Table")]
