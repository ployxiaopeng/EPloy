    ...
    internal sealed partial class ResourceEditor : EditorWindow
    {
        ...
        private void OnGUI()
        {
            EditorGUILayout.BeginHorizontal(GUILayout.Width(position.width), GUILayout.Height(position.height));
            {
                ...
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(UtilText.Format("Resource List ({0})", m_Controller.ResourceCount.ToString()), EditorStyles.boldLabel);
                ...
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(UtilText.Format("Resource Content ({0})", m_CurrentResourceContentCount.ToString()), EditorStyles.boldLabel);
                    EditorGUILayout.BeginHorizontal("box", GUILayout.Height(position.height - 52f));
                    {
                        DrawResourceView();
                    }
        ...
        private void DrawResourcesMenu_Remove()
        {
            ...
            GUILayout.Label(UtilText.Format("Remove '{0}' ?", m_SelectedResource.Name));
        ...
        private void DrawResourceView()
        {
            m_ResourceViewScroll = EditorGUILayout.BeginScrollView(m_ResourceViewScroll);
            {
                ...
                {
                    int index = 0;
                    Asset[] assets = m_Controller.GetAssets(m_SelectedResource.Name);
        ...
        private void DrawResourceMenu()
        {
            ...
            {
                if (GUILayout.Button(UtilText.Format("{0} >>", m_SelectedAssetsInSelectedResource.Count.ToString()), GUILayout.Width(80f)))
                {
        ...
        private void DrawSourceAssetsMenu()
        {
            ...
            {
                if (GUILayout.Button(UtilText.Format("<< {0}", selectedSourceAssets.Count.ToString()), GUILayout.Width(80f)))
                {
            ...
            EditorGUI.BeginDisabledGroup(selectedSourceAssets.Count <= 0);
            {
                if (GUILayout.Button(UtilText.Format("<<< {0}", selectedSourceAssets.Count.ToString()), GUILayout.Width(80f)))
                {
                    ...
                    foreach (SourceAsset sourceAsset in selectedSourceAssets)
                    {
                        EditorUtility.DisplayProgressBar("Add Resources", UtilText.Format("{0}/{1} processing...", (++index).ToString(), count.ToString()), (float)index / count);
                        ...
                        AddResource(sourceAsset.FromRootPath, false);
                        Resource resource = m_Controller.GetResource(name);
                        if (resource == null)
                        {
                            continue;
                        }
        ...
        private void DrawSourceAsset(SourceAsset sourceAsset)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUILayout.LabelField(asset != null ? asset.Resource.Name : string.Empty, GUILayout.Width(position.width * 0.15f));
            }
            EditorGUILayout.EndHorizontal();
            m_CurrentSourceRowOnDraw++;
        }
        ...
        private void AddResource(string fullName, bool refresh)
        {
            if (m_Controller.AddResource(fullName, LoadType.LoadFromFile))
            {
                if (refresh)
                {
                    RefreshResourceTree();
                ...
                Debug.Log(UtilText.Format("Add resource '{0}' success.", name));
                m_MenuState = MenuState.Normal;
            }
            else
            {
                Debug.LogWarning(UtilText.Format("Add resource '{0}' failure.", name));
            }
        }
        ...
        private void RenameResource(Resource resource, string newName)
        {
            ...
            if (m_Controller.RenameResource(oldName, newName))
            {
                RefreshResourceTree();
                Debug.Log(UtilText.Format("Rename resource '{0}' to '{1}' success.", oldName, newName));
                m_MenuState = MenuState.Normal;
            }
            else
            {
                Debug.LogWarning(UtilText.Format("Rename resource '{0}' to '{1}' failure.", newName, oldName));
            }
        }

        private void RemoveResource()
        {
            string name = m_SelectedResource.Name;
            if (m_Controller.RemoveResource(m_SelectedResource.Name))
            {
                ChangeSelectedResource(null);
                RefreshResourceTree();
                Debug.Log(UtilText.Format("Remove resource '{0}' success.", name));
            }
            else
            {
                Debug.LogWarning(UtilText.Format("Remove resource '{0}' failure.", name));
            }
        }

        private void SetResourceLoadType(LoadType loadType)
        {
            string fullName = m_SelectedResource.Name;
            if (m_Controller.SetResourceLoadType(m_SelectedResource.Name, loadType))
            {
                Debug.Log(UtilText.Format("Set resource '{0}' load type to '{1}' success.", fullName, loadType.ToString()));
            }
            else
            {
                Debug.LogWarning(UtilText.Format("Set resource '{0}' load type to '{1}' failure.", fullName, loadType.ToString()));
            }
        }

        private void AssignAsset(SourceAsset sourceAsset, Resource resource)
        {
            if (!m_Controller.AssignAsset(sourceAsset.Guid, resource.Name))
            {
                Debug.LogWarning(UtilText.Format("Assign asset '{0}' to resource '{1}' failure.", sourceAsset.Name, resource.Name));
            }
        }
        ...
        private void UnassignAsset(Asset asset)
        {
            ...
            {
                Debug.LogWarning(UtilText.Format("Unassign asset '{0}' from resource '{1}' failure.", asset.Guid, m_SelectedResource.Name));
            }
        }
        ...
        private void CleanResource()
        {
            ...
            Debug.Log(UtilText.Format("Clean complete, {0} unknown assets and {1} unused resources has been removed.", unknownAssetCount.ToString(), unusedResourceCount.ToString()));
        }
        ...
        private void RefreshResourceTree()
        {
            ...
            foreach (Resource resource in resources)
            {
                ...
                folder.AddItem(resource.Name, resource);
            }
        }
        ...
        {
            return variant != null ? UtilText.Format("{0}.{1}", name, variant) : name;
        }

        private void OnLoadingResource(int index, int count)
        {
            EditorUtility.DisplayProgressBar("Loading Resources", UtilText.Format("Loading resources, {0}/{1} loaded.", index.ToString(), count.ToString()), (float)index / count);
        }

        private void OnLoadingAsset(int index, int count)
        {
            EditorUtility.DisplayProgressBar("Loading Assets", UtilText.Format("Loading assets, {0}/{1} loaded.", index.ToString(), count.ToString()), (float)index / count);
        }
