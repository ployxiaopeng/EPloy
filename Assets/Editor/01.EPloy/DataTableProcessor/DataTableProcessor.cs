    ...
    public sealed partial class DataTableProcessor
    {
        ...
        public DataTableProcessor(string dataTableFileName, Encoding encoding, int nameRow, int typeRow, int? defaultValueRow, int? commentRow, int contentStartRow, int idColumn)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                {
                    throw new GameFrameworkException(Utility.Text.Format("Table:{3},Raw Column is '{1}', but line '{0}' column is '{2}'.", i.ToString(), rawColumnCount.ToString(), rawValue.Length.ToString(), dataTableFileName));
                }
