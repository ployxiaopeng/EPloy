...
namespace EPloy
{
    ...
    public class ILInstanceMethod
    {
        ...
        public ILInstanceMethod(object hotfixInstance, string typeName, string methodName, int paramCount)
        {
            ...
            m_Method = Init.ILRuntimeMgr.AppDomain.LoadedTypes[typeName].GetMethod(methodName, paramCount);
            m_Params = new object[paramCount];
        }

        public void Invoke()
        {
            Init.ILRuntimeMgr.AppDomain.Invoke(m_Method, m_HotfixInstance, m_Params);
        }
