...
namespace EPloy.Table
{
    ...
    public class DataTableHelper 
    {
        ...
        {
            GameModule.Res.LoadBinary(dataTableAssetName, LoadBinaryCallbacks);
        }
        ...
        public bool ParseData(byte[] dataTableBytes, int startIndex, int length)
        {
            ...
            {
                Log.Warning(UtilText.Format("Can not parse dictionary bytes with exception '{0}'.", exception.ToString()));
                return false;
            }
        }
        ...
        {
            string appendErrorMessage = UtilText.Format("Load data failure, data asset name '{0}', status '{1}', error message '{2}'.", dataAssetName, status.ToString(), errorMessage);
            DataTableFailureEvt Evt = ReferencePool.Acquire<DataTableFailureEvt>();
            Evt.SetData(dataAssetName, appendErrorMessage);
            GameModule.Event.Fire(Evt);
        ...
        private void LoadBinarySuccessCallback(string dataAssetName, byte[] dataBytes, float duration)
        {
            try
            {
                ...
                {
                    Log.Fatal(UtilText.Format("Load data failure in data provider helper, data asset name '{0}'.", dataAssetName));
                    return;
                }
